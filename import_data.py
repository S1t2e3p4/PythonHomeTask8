...
def create_database_classes(nameclass=False): 
    if not nameclass:
        nameclass = input('Введите номер класса: ')
    all_classes[nameclass] = []


def create_database_students(): 
    for _ in range(randint(10, 15)):
        global id_student
        global all_students
        global all_classes

        surname = ['Ivanov', 'Petrov', 'Sidarov', 'Shliapin', 'Pupkin', 'Zalupcin',
                   'Konev', 'Bikov', 'Hriakov', 'Mliakov', 'Pontuz', 'Gundosii', 'Tugodumov']
        surname = surname[randint(0, 12)]
        name = ['Viktor', 'Vasili', 'Kolian', 'Sioma', 'Serzj', 'Iraz',
                'Innal', 'Ksen', 'Max', 'Alexei', 'Natal', 'Mark', 'Dmitri']
        name = name[randint(0, 12)]
        patronymic = ['Zimmerovich', 'liahovich', 'Chelovich', 'Bukovich', 'Klimovich',
                      'Penovich', 'Abramovich', 'St.', 'Jn.', 'Ruslanovich', 'Maximovich', 'Alximovich', 'Stepanovich']
        patronymic = patronymic[randint(0, 12)]
        day = randint(1, 31)
        if day < 10:
            day = str(f'0{day}')
        month = randint(1, 12)
...
def create_class(cl_name=False):
    ...
    if not cl_name:
        cl_name = input('Введите номер класса: ')
    all_classes[cl_name] = []
    log.log_input_data('Создан новый класс')
    record_to_file(all_classes)
    print(all_classes)
